...
const app = express();
const PORT = process.env.PORT || 4000; // Railway akan menyediakan process.env.PORT
app.listen(PORT, () => { /* ... */ });

// Middleware
app.use(cors()); // Mengizinkan Cross-Origin Resource Sharing

const allowedOrigins = ['http://localhost:5173']; // Ganti dengan port Vite Anda jika berbeda
